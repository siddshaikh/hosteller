...
import { removeFromCart, addToCart } from "@/store/features/cart/cartSlice";
...
export const Summary = () => {
  ...
  return (
    <div>
      <h1 className="uppercase tracking-wide font-bold text-xl">Summary</h1>
      <div className="flex items-center justify-between border border-gray-300 rounded-md p-1 px-2">
        ...
        <p className="flex flex-col">
          ...
          <span className="font-bold">{format(nextDay, "dd MM, yyyy")}</span>
        </p>
      </div>
      {/* items */}
      {cartItems.length > 0 ? (
      ...
      <div className="flex items-center justify-between">
        ...
        <h4 className="font-bold text-lg">₹{(totalPrice + tax).toFixed(2)}</h4>
      </div>
      {/* checkout */}
